...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        # Check if the order is a rental order
        if self.is_rental_order:
            return super(SaleOrder, self).action_confirm()

        # Continue with minimum price validation for non-rental orders
        ...
        if len(check_product) > 0 and not skip_check_price:
            ...
            product_str_fr = ('\n').join(
                f" {i + 1}. {product.display_name} le prix minimum est {product.currency_id.symbol} {product.minimum_sale_price:.2f}"
                for i, product in enumerate(check_product)
            )
    ...
    def check_product_price(self):
        ...
        for line in self.order_line:
            # Check if line price is less than the product's minimum sale price
